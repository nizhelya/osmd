...
$this->results = '['; 
 ...
 $like='';
 		if ($param3) {
			      ...
			      $filter= ' `sname` like "%'.$param2.'%" OR `inn` like "%'.$param2.'%" OR `edrpou` like "%'.$param2.'%" AND ';
			         }
		else if ($param1) {
			      $filter = ' `cat_id`='.$param1.' AND ';
				  } 
		else {$filter = '';}
		$this->sql='SELECT `org_id`, `sname`, `fname`, `cat_id`, `org_id` FROM TM_ORG WHERE '.$filter.' `cat_id`>=0 LIMIT 50'; /*  (TM_ORG.vibor ="utke" OR TM_ORG.vibor ="all") */
		//print_r($this->sql); 
...
$_result_articles = $_db->query('SELECT * FROM TM_ORG_FILIAL WHERE org_id = '.$row_company['org_id'].'') or die('Connect Error (' . $_db->connect_errno . ') ' . $_db->connect_error);
$row=mysqli_affected_rows($_db);
$this->results.='subcategories:"'.$row.'",';

$child2 =1;  
		    $_result_depts = $_db->query('SELECT * FROM TM_ORG_FILIAL WHERE org_id = '.$row_company['org_id'].'') or die('Connect Error (' . $_db->connect_errno . ') ' . $_db->connect_error);
